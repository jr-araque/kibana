...
import type { BulkUpdateExceptionListItemsRequestBodyInput } from '@kbn/securitysolution-exceptions-common/api/bulk_update_exception_list_items/bulk_update_exception_list_items.gen';
...
const securitySolutionApiServiceFactory = (supertest: SuperTest.Agent) => ({
  /**
   * Update multiple exception list items in a single request.
   */
  bulkUpdateExceptionListItems(
    props: BulkUpdateExceptionListItemsProps,
    kibanaSpace: string = 'default'
  ) {
    return supertest
      .put(getRouteUrlForSpace('/api/exception_lists/items/_bulk', kibanaSpace))
      .set('kbn-xsrf', 'true')
...
> All exception items added to the same list are evaluated using `OR` logic. That is, if any of the items in a list evaluate to `true`, the exception prevents the rule from generating an alert. Likewise, `OR` logic is used for evaluating exceptions when more than one exception list is assigned to a rule. To use the `AND` operator, you can define multiple clauses (`entries`) in a single exception item.
   ...
   */
  readExceptionListSummary(props: ReadExceptionListSummaryProps, kibanaSpace: string = 'default') {
    return supertest
...
export interface BulkUpdateExceptionListItemsProps {
  body: BulkUpdateExceptionListItemsRequestBodyInput;
}
